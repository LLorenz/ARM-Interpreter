...
var commandMap = (function () {
	...
	function evalFlexibleOperatorFunction(flexOpFirstPart, flexOpSecondPart, writeStatus) {
		...
		var flexOpSecondPartString = flexOpSecondPart[1];
		...
		case "ASR":
			...
			}, shiftByRegister, shiftByRegister ? 0x000000010000 | parseRegister(flexOpSecondPartString) << 8 | shiftType << 5 | parseRegister(flexOpFirstPart)
																					: 0x000000000000 | flexOpSecondPartString << 7 | shiftType << 5 | parseRegister(flexOpFirstPart) ]
		case "LSR":
			shiftType = 1;
			return [function () {
				var value = firstPartValue();
