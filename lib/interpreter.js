...
var commandMap = (function () {
	...
	function ldrStrCalculateAddress(source, offset, shift) {
		...
		if (arguments.length == 1 && source.charAt(0) == "[") {
			...
			var offsetFunction = evalFlexibleOperatorFunction(offset, shift, true)[0];
	...
	populateCommandMap("LDR<cond>", function (result, source, offset) {
		...
		return function () {
			if (reference.type == "indirect") {
				// assume little endian
				setResult(memory[reference.address()] +
					  memory[reference.address() + 1] * Math.pow(2, 8) +
					  memory[reference.address() + 2] * Math.pow(2, 16) +
					  memory[reference.address() + 3] * Math.pow(2, 24));
			} else {
				setResult(reference.address());
			}
		}
	});
	populateCommandMap("STR<cond>", function (result, source, offset) {
		...
		return function () {
			if (reference.type == "indirect") {
				// assume little endian
				setMemoryAddress(reference.address(), getResult() % Math.pow(2, 8));
				setMemoryAddress(reference.address() + 1, getResult() >>> 8 % Math.pow(2, 8));
				setMemoryAddress(reference.address() + 2, getResult() >>> 16 % Math.pow(2, 8));
				setMemoryAddress(reference.address() + 3, getResult() >>> 24 % Math.pow(2, 8));
			} else {
...
console.log(returner);
	...
	function branch(link, whereTo) {
		...
		var baseAddress = this.memoryAddress + 4; // ARM Architecture Reference Manual says 8, but that seems to be wrong... 32 bit instruction width => 4 bytes
		var offset = whereToContent - baseOffset;
		...
		if (!negative) {
			...
			immediateSignedThreeByte = convToUInt32(~offset & 0xFFF + 1);
		}
		...
		return {
			...
			bytecode: 0b101 << 25 | link << 24 | immediateSignedThreeByte;
		}
	}
	...
	function branchExchange(link, whereTo) {
		...
		return {
			...
			bytecode: 0b00010010 << 20 | 0xFFF << 8 | 1 << 4 | parseRegister(whereTo) | link << 5
		}
	}
...
function Command(commandString, memoryAddress, lineNumber) {
	...
	// ugly hack: We sometimes need to give additional information to the command factory. This is done via this. For example, B and BL need the memory address to calculate the offset of the target address.
	var command = commandFactory.apply({
