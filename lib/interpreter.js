...
var commandMap = (function () {
	...
	function ldrStrCalculateAddress(source, offset) {
		...
		if (arguments.length == 1 && source.charAt(source.length - 1) == "]") {
			...
			return {
				type: 'indirect',
				address: getLabelFunction(source.trim())
			}
		} else {
			// offset
			...
			var offsetFunction;
			try {
				offsetFunction = getRegisterFunction(offset.substr(0, offset.length - 1));
			} catch (e) {
				offsetFunction = parseNumericConstant(offset.substr(0, offset.length - 1));
			}
			return function() {
				return {
					type: 'indirect',
					address: position() + offsetFunction()
				};
			}
		}
	}
