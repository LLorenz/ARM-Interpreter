...
var commandMap = (function () {
	...
	function evalFlexibleOperatorFunction(flexOpFirstPart, flexOpSecondPart, writeStatus) {
		...
		if (constant != undefined) {
			...
			if (shiftBitCounter == 16) {
			 throw new ParseException("Illegal Immediate Value - Only  8-bit rotated by an even number of bits allowed by ARM Standard.");
				...
				return function () {
					return {constant, 1, bytecode: 0x000000000000 | shiftBitCounter << 8 | value};
				}
			}			
		}
		...
		if (!flexOpSecondPart) {
			...
			return function () {
				return {firstPartValue(), 0, bytecode: 0x000000000000 | parseRegister(flexOpFirstPart)};
			}
		}
		...
		if (flexOpSecondPart.length == 1) {
			...
			if (flexOpSecondPart[0].toUpperCase() == "RRX") {
				...
				}, 0, bytecode: 0x000001100000 | parseRegister(flexOpFirstPart)}
			}
		}
		...
		case "ASR":
			...
			}, shiftByRegister, shiftByRegister ? bytecode: 0x000000010000 | parseRegister(flexOpSecondPart[1]) << 8 | shiftType << 5 | parseRegister(flexOpFirstPart) : bytecode: 0x000000000000 | parseRegister(flexOpSecondPart[1]) << 7 | shiftType << 5 | parseRegister(flexOpFirstPart) }
		case "LSR":
			shiftType = 1;
			return {function () {
				var value = firstPartValue();
