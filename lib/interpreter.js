...
var commandMap = (function () {
			...
			conditionCodes.forEach(function (conditionArray) {
				populateCommandMapConditionsSolved(name.replace("<cond>", conditionArray[0]), function () {
					...
					return {
						...
						bytecode: operation.bytecode | (conditionArray[1] << 28)
					}
				});
			});
		}
	...
	function setRegisterFunction(registerString) {
		...
		return function (value) {
			registers[registerIndex] = (registerIndex == 15) ? value - 4 : value;
		}
	}
	...
	function evalFlexibleOperatorFunction(flexOpFirstPart, flexOpSecondPart, writeStatus) {
		...
		if (constant != undefined) {
			...
			if (shiftBitCounter == 16) {
			 throw new ParseException("Illegal Immediate Value - Only  8-bit rotated by an even number of bits allowed by ARM Standard.");
				...
				return [function () {
					return constant;
				}, 1, 0x000000000000 | shiftBitCounter << 8 | value];
			}			
		}
		...
		if (!flexOpSecondPart) {
			...
			}, 0, 0x000000000000 | parseRegister(flexOpFirstPart)];
		}
		...
		if (flexOpSecondPart.length == 1) {
			...
			if (flexOpSecondPart[0].toUpperCase() == "RRX") {
				...
				}, 0, 0x000001100000 | parseRegister(flexOpFirstPart)]
			}
		}
		...
		case "ASR":
			...
			return [function () {
				...
				if (typeof(flexOpSecondPart[1]) === 'function') {
					...
					if (flexOpSecondPart[1] > Math.pow(2,5) - 1) {
			...
			}, shiftByRegister, shiftByRegister ? 0x000000010000 | parseRegister(flexOpSecondPart[1]) << 8 | shiftType << 5 | parseRegister(flexOpFirstPart) : 0x000000000000 | parseRegister(flexOpSecondPart[1]) << 7 | shiftType << 5 | parseRegister(flexOpFirstPart)]
		case "LSR":
			shiftType = 1;
			return [function () {
				var value = firstPartValue();
	...
	function arith(operatorBits, arithOperator, writeStatus, result, firstOp, flexOpFirstPart, flexOpSecondPart) {
		...
		return {
			...
			bytecode: 0x00000000000000000000000000000000 | secondaryI << 25 | operatorBits << 21 | writeStatus << 20 | parseRegister(result) << 16 | parseRegister(firstOp) << 12 | secOpEval[2] << 0
		};
	}
	 ...
	 */
	 function mul(operatorBits, arithOperator, writeStatus, result, firstOp, secondOp) {
		...
		return {
			...
			bytecode: 0x00345678 | operatorBits << 21
		};
	}
	[
		["MUL", function(writeStatus,first,second){
			var result = convToUInt32(first * second);
			if (writeStatus) {
	...
	function comp(operatorBits, compOperator, firstOp, flexOpFirstPart, flexOpSecondPart) {
		...
		var secOpEval = evalFlexibleOperatorFunction(flexOpFirstPart, flexOpSecondPart, true);
		var getSecondOp = secOpEval[0];
		var secondaryI = secOpEval[1];
		var secondaryBC = secOpEval[2];
		return {
			...
			bytecode: 0x00000000000000000000000000000000 | secondaryI << 25 | operatorBits << 21 | writeStatus << 20 | parseRegister(firstOp) << 12 | secOpEval[2] << 0
		};
	}
	[
	...
	function mov(operatorBits, movOperator, writeStatus, result, flexOpFirstPart, flexOpSecondPart) {
		...
		return {
			...
			bytecode: 0x00000000000000000000000000000000 | secondaryI << 25 | operatorBits << 21 | writeStatus << 20 | parseRegister(result) << 16 | secOpEval[2] << 0
		};
	}
	...
	function ldrStrCalculateAddress(source, offset, shift) {
		assert (source.charAt(0) == "[" || source.charAt(0) == "=", "LDR/STR instruction not recognized") 
		assert(arguments.length >= 1 && arguments.length <= 3, "Argument count wrong, expected 2, 3 or 4, got " + arguments.length + 1); // +1 because first argument is destination => cut off for processing here

		if (arguments.length == 1 && source.charAt(0) == "[") {
			assert (source.charAt(source.length - 1) == "]");
			return {
				type: 'indirect',
				address: getRegisterFunction(source.substr(1, source.length - 2).trim())
			};
		} else if (arguments.length == 1 && source.charAt(0) == "=") {
			// label
			return {
				type: 'direct',
				address: getLabelFunction(source.substr(1).trim())
			}
		} else if (arguments.length == 1) {
			// content at address of label
			return {
				type: 'indirect',
				address: getLabelFunction(source)
			}
		} else {
			// some sort of offset
			var mode;
			if (source.charAt(source.length - 1) == "]") {
				// post-indexed offset ([Ra], Rb)
				mode = 1;
				source = source.substr(1, source.length - 2).trim();
			} else if (arguments.length == 2 && offset.charAt(offset.length - 1) == "]") {
				// immediate offset ([Ra, Rb]) without shift
				mode = 2;
				source = source.substr(1, source.length - 1).trim();
				offset = offset.substr(0, offset.length - 1).trim();
			} else if (arguments.length == 3 && shift.charAt(shift.length - 1) == "]") {
				// immediate offset ([Ra, Rb]) without shift
				mode = 2;
				source = source.substr(1, source.length - 1).trim();
				shift = shift.substr(0, shift.length - 1).trim();
			} else if (arguments.length == 2 && offset.match(/]\s*!$/)) {
				// pre-indexed offset ([Ra, Rb]!)
				mode = 3;
				source = source.substr(1, source.length - 1).trim();
				offset = offset.replace(/]\s*!$/, ""); // no substr because we don't know amount of white space between ] and !
			} else if (arguments.length == 3 && shift.match(/]\s*!$/)) {
				// pre-indexed offset ([Ra, Rb]!)
				mode = 3;
				source = source.substr(1, source.length - 1).trim();
				shift = shift.replace(/]\s*!$/, ""); // no substr because we don't know amount of white space between ] and !
			} else {
				assert(false, "LDR/STR instruction not recognized");
			}
			var position = parseRegister(source);
			
			var isOffsetNegative = false;
			if (offset.charAt(0) == '+') {
			...
			var offsetFunction = evalFlexibleOperatorFunction(offset, shift, true);
			
			if (isOffsetNegative) {
				var originalOffsetFunction = offsetFunction;
				offsetFunction = function() {
					return -originalOffsetFunction.apply(arguments);
				}
			}
			...
			return function() {
				...
				switch (mode) {
					case 3:
					// pre-indexed
					registers[position] += offsetFunction();
					address = registers[position];
					break;
					case 2:
					// immediate
					address = registers[position] + offsetFunction();
					break;
					case 1:
					// post-indexed
					address = registers[position];
	...
	populateCommandMap("LDR<cond>", function (result, source, offset) {
		...
		return function () {
			...
			if (ref.type == "indirect") {
				...
				setResult(memory[ref.address()] +
					  memory[ref.address() + 1] * Math.pow(2, 8) +
					  memory[ref.address() + 2] * Math.pow(2, 16) +
					  memory[ref.address() + 3] * Math.pow(2, 24));
			} else {
				setResult(ref.address());
			}
		}
	});
	populateCommandMap("STR<cond>", function (result, source, offset) {
		...
		return function () {
			...
			if (ref.type == "indirect") {
				// assume little endian
				setMemoryAddress(ref.address, getResult() % Math.pow(2, 8));
				setMemoryAddress(ref.address + 1, getResult() >>> 8 % Math.pow(2, 8));
				setMemoryAddress(ref.address + 2, getResult() >>> 16 % Math.pow(2, 8));
				setMemoryAddress(ref.address + 3, getResult() >>> 24 % Math.pow(2, 8));
			} else {
				assert(false, "Trying to use a label with a STR opcode. This is illegal ARM code.")
			}
		}
	});
...
function Command(commandString, memoryAddress, lineNumber) {
	...
	return [command.func, dummyCommand, dummyCommand, dummyCommand];
}
