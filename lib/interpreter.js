...
var commandMap = (function () {
	...
	populateCommandMap("LDR<cond>", function (result, source, offset) {
		assert(arguments.length == 2 || arguments.length == 3, "Argument count wrong, expected 2 or 3, got " + arguments.length);
		assert(arguments.length == 2, "Sorry, offsets aren't supported yet"); // everything except labels is indirectand in []. TODO

		assert((arguments.length == 2 && (source.charAt(source.length - 1) == "]") || (source.charAt(0) == "="), "LDR instruction not recognized")
	  ...
	  var setResult = setRegisterFunction(result);
		if (source.charAt(source.length - 1) == "]") {
			...
			// label
			console.log("Getting label for ",source.substr(1).trim())
			var getLabel = getLabelFunction(source.substr(1).trim());
			return function() {
				setResult(getLabel());
			}
		}
...
function Command(commandString, lineNumber) {
	...
	if (commandString.substr(0, 7).toLowerCase() == ".asciz ") {
		...
		for (i = 0; i < string.length; i++) {
			memory[lineNumber + i] = string.charCodeAt(i);
		}
		memory[lineNumber + i] = 0;
		return;
	}
	if (opcodeLength == -1) {
			...
			for (var i = 0; i < formatString.length; i++) {
				if (formatString.substr(i, 1) != "%") {
					...
					switch (formatString.substr(i, 1)) {
						case 'i':
						case 'd':
							returner += (memory[currentRegister] +
							             memory[currentRegister + 1] * Math.pow(2, 8) +
											     memory[currentRegister + 2] * Math.pow(2, 16) +
											     memory[currentRegister + 3] * Math.pow(2, 24));
					    currentRegister++;
							break;
						case 's':
							returner += stringFromAddress(currentRegister);
							break;
					}
				}
			}
