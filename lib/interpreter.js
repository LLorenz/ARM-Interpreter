...
var commandMap = (function () {
	...
	function arith(operatorBits, arithOperator, writeStatus, result, firstOp, flexOpFirstPart, flexOpSecondPart) {
		...
		return {
			...
			bytecode: 0x00345678 | operatorBits << 24
		}
	}

	[
		["ADD", 0b0000, function (writeStatus, first, second) {
		...
		["SUB", 0b0000, function (writeStatus, first, second) {
				var result = convToUInt32(first - second);
				if (writeStatus) {
		...
		["RSB", 0b0000, function (writeStatus, first, second) {
		...
		["ADC", 0b0000, function (writeStatus, first, second) {
		...
		["SBC", 0b0000, function (writeStatus, first, second) {
				if (flags.CARRY) {
					var result = convToUInt32(first - second);
				} else {
					var result = convToUInt32(first - second - 1);
		...
		["RSC", 0b0000, function (writeStatus, first, second) {
				if (flags.CARRY) {
					var result = convToUInt32(second - first);
				} else {
	 ...
	 *
	 */
	 function mul(arithOperator, writeStatus, result, firstOp, secondOp) {
	...
	[
		...
		populateCommandMap(array[0] + "<cond><S>", mul.bind(null, array[1]));
	});
